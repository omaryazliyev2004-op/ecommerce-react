    ...
    <main className="bg-white">
      ...
      <section className="mx-auto -mt-10 grid max-w-7xl grid-cols-1 gap-4 px-5 sm:px-8 md:grid-cols-3 lg:px-10">
        {categories.map((category) => (
          <Link
            key={category.title}
            to={category.to}
            className="group overflow-hidden rounded-[28px] border border-black/5 bg-[#f5f5f7] p-6 shadow-[0_20px_70px_rgba(15,23,42,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_80px_rgba(15,23,42,0.14)]"
          >
            <div className="flex min-h-[210px] items-end justify-between gap-5">
              <div className="max-w-[190px] self-start">
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-gray-400">Explore</p>
                <h2 className="mt-3 text-3xl font-black tracking-normal text-gray-950">{category.title}</h2>
                <p className="mt-3 text-sm leading-6 text-gray-500">{category.text}</p>
              </div>
              <img
                src={category.image}
                alt={category.title}
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-10">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.22em] text-blue-600">Curated selection</p>
            <h2 className="mt-3 text-4xl font-black tracking-normal text-gray-950 md:text-5xl">
              Apple Products
            </h2>
          ...
          <Link
            to="/iphone"
            className="inline-flex min-h-11 items-center justify-center rounded-full bg-gray-950 px-6 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-blue-600"
          >
            Browse catalog
          </Link>
        </div>
